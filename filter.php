...
$Marka = $_POST['marka']; $Fiyat = POST('fiyataraligi'); 
$Kategori =  POST('kategori'); $No =  POST('no'); 
foreach($Marka as $Key => $Yazdir){ $Ayir = implode (",",$Marka);}
if($Fiyat==1){ $SqlSonuc = "AND urunfiyat BETWEEN 100 AND 500 ORDER BY urunfiyat ASC";
}elseif($Fiyat==2){ $SqlSonuc = "AND urunfiyat BETWEEN 500 AND 1000 ORDER BY urunfiyat ASC";
}elseif($Fiyat==3){ $SqlSonuc = "AND urunfiyat BETWEEN 1000 AND 2500 ORDER BY urunfiyat ASC";
}elseif($Fiyat==4){ $SqlSonuc = "AND urunfiyat BETWEEN 2500 AND 5000 ORDER BY urunfiyat ASC";
}elseif($Fiyat==5){ $SqlSonuc = "AND urunfiyat BETWEEN 5000 AND 999999999 ORDER BY urunfiyat ASC";
}else{ $SqlSonuc="";}
$Bir = $Baglan->query("SELECT * FROM hddizayn_kategori WHERE durum = '1' and id = '$Kategori' and no = '$No'");
$iki = $Baglan->query("SELECT * FROM hddizayn_kategori_alt WHERE durum = '1' and id = '$Kategori' and no = '$No'");
$Uc = $Baglan->query("SELECT * FROM hddizayn_categori_alt WHERE durum = '1' and id = '$Kategori' and no = '$No'");
if($Bir->num_rows == 1){$Bir = $Baglan->query("SELECT * FROM hddizayn_kategori WHERE durum = '1' and id = '$Kategori' and no = '$No'");
$Sonuc = $Bir->fetch_object(); $KategoriSonuc = "kategori";
}elseif($iki->num_rows==1){$iki = $Baglan->query("SELECT * FROM hddizayn_kategori_alt WHERE durum = '1' and id = '$Kategori' and no = '$No'");
$Sonuc = $iki->fetch_object(); $KategoriSonuc = "kategori_alt";
}elseif($Uc->num_rows==1){$Uc = $Baglan->query("SELECT * FROM hddizayn_categori_alt WHERE durum = '1' and id = '$Kategori' and no = '$No'");
$Sonuc = $Uc->fetch_object(); $KategoriSonuc = "kategori_altt";
}if(empty($Marka)){$Sorgu = $Baglan->query("SELECT * FROM hddizayn_urunler WHERE durum = 1 AND $KategoriSonuc = $Sonuc->id $SqlSonuc");
}elseif(empty($Fiyat)){$Sorgu = $Baglan->query("SELECT * FROM hddizayn_urunler WHERE durum = 1 AND marka IN ($Ayir)");
}else{$Sorgu = $Baglan->query("SELECT * FROM hddizayn_urunler WHERE durum = 1 AND marka IN ($Ayir) AND $KategoriSonuc = $Sonuc->id  $SqlSonuc");}
?>
<!DOCTYPE html>
<html lang="tr">
<head>
	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	<title>Filitreleme Sonuçları - <?php echo $HDDizayn['title'];?></title>
                ...
                <div class="row">
                    <div class="col-md-12 col-main">
                        <div class=" toolbar-products toolbar-top">
                            <h1 class="cate-title" style="text-transform:uppercase; font-size:20px;">Filitrelenen Ürün (<?php echo $Sorgu->num_rows;?>)</h1>
                            <div class="modes">
